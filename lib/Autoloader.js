...
module.exports = class Autoloader {
  ...
  readDir(path = '') {
    for (let file of fs.readdirSync(resolve(this.basePath, path))) {
      ...
      if (fs.lstatSync(resolve(this.basePath, path, file)).isFile()) {
  ...
  constructModuleNamespace(path, file) {
    const filename = file.split('.').filter(ext => !this.include.includes(ext)).join('.');

    const ext = file.split('.')[file.split('.').length - 1];

    if (!this.include.includes(ext)) {
      return;
    }
    ...
    for (let i = 0, folder; folder = folders[i]; i++) {
      // eslint-disable-line
      if (folder === '') {
